...
module I = struct
  type 'a t =
    { clock : 'a
    ; clear : 'a
    ; part1 : 'a [@bits width_bits]
    ; part2 : 'a [@bits width_bits]
    ; done_ : 'a
    ; uart_tx_ready : 'a
    }
  [@@deriving hardcaml]
...
module O = struct
  type 'a t = { uart_tx : 'a Byte_with_valid.t } [@@deriving hardcaml]
end
...
let bin_to_decimal ~clock ~clear ~start x =
  let open Always in
  ...
  let byte =
    ...
    let mod10 =
      Unsigned.(temp_reg -: (div10_reg *: of_unsigned_int ~width:4 10))
      |> sel_bottom ~width:4
    in
  ...
  compile
    [ mod10_valid <-- gnd
    ; sm.switch
        [ Idle, [ when_ start [ temp <-- x; counter <--. 0; sm.set_next Active ] ]
        ; ( Active
...
let create scope ({ clock; clear; part1; part2; done_; uart_tx_ready } : _ I.t) : _ O.t =
  let spec = Reg_spec.create ~clock ~clear () in
  let part1_conversion_done, part1_reg =
    bin_to_decimal ~clock ~clear ~start:done_ part1
  in
  let part2_conversion_done, part2_reg =
    bin_to_decimal ~clock ~clear ~start:done_ part2
  in
  ...
  let enable = uart_tx_ready &: part1_conversion_done &: part2_conversion_done in
  let counter =
